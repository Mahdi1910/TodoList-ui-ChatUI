...
assert.match(
  mainCss,
  /button:focus-visible,[\s\S]*a:focus-visible,[\s\S]*\[tabindex\]:focus-visible,[\s\S]*input:focus-visible,[\s\S]*textarea:focus-visible,[\s\S]*select:focus-visible[\s\S]*outline:\s*2px solid var\(--accent-blue\)/,
  'keyboard focus must remain visible across buttons, links, custom focusable rows, and native form controls'
);
