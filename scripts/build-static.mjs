...
for (const app of ['ChatUI', 'TodoList-ui']) {
...
for (const directory of ['css', 'html', 'js']) {
  await cp(path.join(root, 'ChatUI', directory), path.join(dist, 'ChatUI', directory), { recursive: true });
}

await copyFile(path.join(root, 'TodoList-ui', 'index.html'), path.join(dist, 'TodoList-ui', 'index.html'));
for (const directory of ['css', 'js']) {
  await cp(path.join(root, 'TodoList-ui', directory), path.join(dist, 'TodoList-ui', directory), { recursive: true });
}
